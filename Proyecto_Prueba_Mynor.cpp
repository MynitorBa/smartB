...
void inicializarUsuarios() {
    ...
    // Agrega mas usuarios segun sea necesario
}
        ...
        if (intentos == 2) {
            for (auto u : usuarios) {
                cout << "El usuario " << usuario
                    << " ha sido bloqueado por demasiados intentos fallidos." << endl;
...
// Funcion para inicializar datos de productos
...
// Función para mostrar el menú de tablets
...
// Función para mostrar el menú de laptops
...
// Función para mostrar el menú de smartphones
...
// Funcion para modificar las existencias de los smartphones
...
// Funcion para modificar las existencias de las laptops
void modificarExistenciasLaptop() {
    ...
    cout << "No se encontro ninguna laptop con el numero de serie proporcionado."
        << endl;
}

// Funcion para modificar las existencias de las tablets
void modificarExistenciasTablet() {
    ...
    cout << "No se encontro ninguna tablet con el numero de serie proporcionado."
        << endl;
}

// Funcion para mostrar el menu de modificacion de existencias

// Función para mostrar el menú de modificación de existencias
...
// Función para mostrar laptops de una marca específica
void mostrarLaptopsPorMarca() {
    ...
    for (const auto& laptop : laptops) {
        if (find(marcasUnicas.begin(), marcasUnicas.end(), laptop.marca) ==
            marcasUnicas.end()) {
...
// Función para mostrar smartphones con mas de 10 existencias
void reporteSmartphonesMasDe10Existencias() {
    limpiarPantalla();
    cout << "Smartphones con más de 10 existencias en inventario:" << endl;
...
// Función para mostrar listado de tablets ordenado por precio
...
// Función para mostrar laptops con SSD de más de 512GB
...
// Función para mostrar listado de smartphones con procesador superior
...
void listaSmartphonesConRam4GBOMas() {
    limpiarPantalla();
    cout << "Smartphones con RAM de 4GB o más:" << endl;
    for (const auto& smartphone : smartphones) {
        if (smartphone.ram >= 4) {
            cout << "Marca: " << smartphone.marca << ", Modelo: " << smartphone.modelo
...
// Función para mostrar tablets con ROM de 64GB o más
void mostrarTabletsConRom64Omas() {
    limpiarPantalla();
    cout << "Tablets con ROM de 64GB o más:" << endl;
...
// Función para mostrar tablets con sistema operativo Android
...
// Función para mostrar listado de laptops gamers en existencias
...
// Funcion para mostrar el submenu del modulo de administrador
void mostrarMenuAdministrador() {
    ...
    for (const auto& Usuario : usuarios) {
        if (Usuario.usuario == "Admin") { // Verificar si el usuario es Admin
            ...
            cout << "5. Volver al menu principal" << endl;
            cout << "Ingrese una opcion: ";
            return; // Salir de la función después de mostrar el menú del
            // administrador
        }
        else {
...
// Funcion para mostrar el submenu del modulo de ventas
...
// Funcion para mostrar tablets con RAM superior a un valor dado
void mostrarTabletsConRamSuperior(int valorRam) {

    cout << "Tablets con RAM superior a " << valorRam << "GB:" << endl;
    for (const auto& tablet : tablets) {
        if (tablet.ram > valorRam) {
            cout << "Marca: " << tablet.marca << ", Modelo: " << tablet.modelo
                << ", RAM: " << tablet.ram << "GB" << endl;
        }
    }
}

// Funcion para controlar accesos de usuarios
void controlDeAccesos() {
    ...
    if (usuarios[0].usuario == "Admin") {
        ...
        cout << "Acceso denegado. Esta función solo esta disponible para el "
            "administrador."
            << endl;
    }
}

// Funcion para modificar un smartphone en el inventario
void modificarSmartphone(int numeroSerie) {
    ...
    for (int i = 0; i < smartphones.size(); ++i) {
        if (smartphones[i].numSerie == numeroSerie) {
            ...
            cin >> smartphones[i].anioFabricacion;
            ...
            cin >> smartphones[i].rom;
            cout << "Gama: ";
            getline(cin >> ws, smartphones[i].gama);
            cout << "Precio: ";
            cin >> smartphones[i].precio;
            cout << "Existencias: ";
            cin >> smartphones[i].existencias;
            cout << "Camara frontal (MP): ";
            cin >> smartphones[i].camaraFrontal;
            cout << "Camara trasera (MP): ";
            cin >> smartphones[i].camaraTrasera;
            cout << "Tamanno de pantalla (pulgadas): ";
            cin >> smartphones[i].pantalla;
...
// Funcion para modificar una laptop en el inventario
...
// Funcion para modificar una tablet en el inventario
void modificarTablet(int numeroSerie) {
    ...
    for (int i = 0; i < tablets.size(); ++i) {
        if (tablets[i].numSerie == numeroSerie) {
            ...
            cin >> tablets[i].rom;
            cout << "Precio: ";
            cin >> tablets[i].precio;
            cout << "Tamanno de pantalla (pulgadas): ";
            cin >> tablets[i].pantalla;
            cout << "Existencias: ";
            cin >> tablets[i].existencias;
...
// Funcion para agregar un smartphone al inventario
void agregarSmartphone() {
    ...
    cin >> nuevoSmartphone.anioFabricacion;
    cin.ignore(numeric_limits<streamsize>::max(), '\n');
...
// Funcion para agregar una laptop al inventario
void agregarLaptop() {
    limpiarPantalla();
...
// Funcion para agregar una tablet al inventario
void agregarTablet() {
    limpiarPantalla();
...
// Funcion para quitar un smartphone del inventario
...
// Funcion para quitar una laptop del inventario
void quitarLaptop() {
    limpiarPantalla();
...
// Funcion para quitar una tablet del inventario
void quitarTablet() {
    limpiarPantalla();
...
// Funcion para quitar un producto del inventario
void quitarProducto() {
    char opcionCategoria;
...
// Funcion para modificar un producto en el inventario
...
void agregarSmartphoneAOrden() {
    ...
    for (const auto& smartphone : smartphones) {
        if (smartphone.numSerie == numSerie) {
            ...
            if (cantidad > smartphone.existencias) {
                cout << "No hay suficientes existencias del smartphone seleccionado."
                    << endl;
            }
            else {
                OrdenDetalle detalle = { "Smartphone", smartphone.marca,
                                        smartphone.modelo, cantidad, smartphone.precio };
                orden.push_back(detalle);
                cout << "El smartphone se ha agregado a la orden correctamente."
                    << endl;
                encontrado = true;
            }
            break;
    ...
    if (!encontrado) {
        cout << "No se encontro un smartphone con el numero de serie proporcionado."
            << endl;
    }
...
void agregarLaptopAOrden() {
    ...
    for (const auto& laptop : laptops) {
        if (laptop.numSerie == numSerie) {
            ...
            if (cantidad > laptop.existencias) {
                cout << "No hay suficientes existencias de la laptop seleccionada."
                    << endl;
            }
            else {
                OrdenDetalle detalle = { "Laptop", laptop.marca, laptop.modelo, cantidad,
                                        laptop.precio };
                orden.push_back(detalle);
    ...
    if (!encontrado) {
        cout << "No se encontro una laptop con el numero de serie proporcionado."
            << endl;
    }
    cout << "Presione cualquier tecla para continuar...";
    _getch();
...
void agregarTabletAOrden() {
    ...
    for (const auto& tablet : tablets) {
        if (tablet.numSerie == numSerie) {
            ...
            if (cantidad > tablet.existencias) {
                cout << "No hay suficientes existencias de la tablet seleccionada."
                    << endl;
            }
            else {
                OrdenDetalle detalle = { "Tablet", tablet.marca, tablet.modelo, cantidad,
                                        tablet.precio };
                orden.push_back(detalle);
    ...
    if (!encontrado) {
        cout << "No se encontro una tablet con el numero de serie proporcionado."
            << endl;
    }
    cout << "Presione cualquier tecla para continuar...";
    _getch();
...
void imprimirFactura() {
    ...
    if (orden.empty()) {
        ...
        // Iterar sobre cada detalle en la orden
        ...
        cout << "------------------------------------------------------------------"
            "-----------------------------------"
            << endl;
        cout << setw(70) << "Total de la factura: $" << totalFactura << endl;
        cout << "=================================================================="
            "=================================="
            << endl;

        // almacenar factura
        facturas.push_back({ numeroFactura, totalFactura });

        for (auto detalle : orden) {
            // Calcular el subtotal para este detalle
            double subtotal = detalle.cantidad * detalle.precioUnitario;

            // Buscar el producto en el inventario de tablets y restar la cantidad
            for (auto& producto : tablets) {
                if (producto.modelo == detalle.modelo &&
                    producto.marca == detalle.marca) { // Corregir la comparación aquí
                    producto.existencias -= detalle.cantidad;

                    // Eliminar producto si tiene 0 existencias
                    if (producto.existencias <= 0) {
                        tablets.erase(remove_if(tablets.begin(), tablets.end(),
                            [&](const Tablet& t) {
                                return t.modelo == detalle.modelo &&
                                    t.marca == detalle.marca;
                            }),
                            tablets.end());
                    }
                    break; // Salir del bucle una vez que se haya actualizado la
                    // existencia
                }
            }
        }
        ...
        for (auto detalle : orden) {
            ...
            // Buscar el producto en el inventario de laptops y restar la cantidad
            for (auto& producto : laptops) {
                if (producto.modelo == detalle.modelo &&
                    producto.marca == detalle.marca) { // Corregir la comparación aquí
                    producto.existencias -= detalle.cantidad;

                    // Eliminar el producto si tiene 0 existencias
                    if (producto.existencias <= 0) {
                        laptops.erase(remove_if(laptops.begin(), laptops.end(),
                            [&](const Laptop& l) {
                                return l.modelo == detalle.modelo &&
                                    l.marca == detalle.marca;
                            }),
                            laptops.end());
                    }
                    break; // Salir del bucle una vez que se haya actualizado la
        ...
        for (auto detalle : orden) {
            ...
            // Buscar el producto en el inventario de smartphones y restar la cantidad
            for (auto& producto : smartphones) {
                if (producto.modelo == detalle.modelo &&
                    producto.marca ==
                    detalle.marca) { // La comparación parece estar bien
                    producto.existencias -= detalle.cantidad;

                    // Eliminar el producto si tiene 0 existencias
                    if (producto.existencias <= 0) {
                        smartphones.erase(remove_if(smartphones.begin(), smartphones.end(),
                            [&](const Smartphone& s) {
                                return s.modelo == detalle.modelo &&
                                    s.marca == detalle.marca;
                            }),
                            smartphones.end());
                    }
                    break; // Salir del bucle una vez que se haya actualizado la
        ...
        // Limpiar la orden después de generar la factura
        orden.clear();

        // Presionar para avnzar
        cout << "Recomendamos tomar captura de la factura, ya que sera borrada "
            "para poder imprimir la de la siguiente orden."
            << endl;
        cout << "Presione cualquier tecla para continuar..." << endl;
        _getch();
    }
...
// Función para imprimir todas las facturas almacenadas en el vector
                        ...
                        do {
                            ...
                            case '1': {
                                while (true) {
                                    ...
                                    case '1':
                                        int ramTablet;
                                        cout << "Ingrese el valor de RAM: ";
                                        cin >> ramTablet;
                                        mostrarTabletsConRamSuperior(ramTablet);
                                        break;
                                    case '2':
